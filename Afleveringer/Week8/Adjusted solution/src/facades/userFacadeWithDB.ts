...
export default class UserFacade {
    ...
    static async deleteUser(userName: string): Promise<string> {
        ...
        if (!status.value) {
            throw new ApiError(`User ${userName} not found and not deleted`, 404)
        }
    ...
    static async getAllUsers(): Promise<Array<any>> {
        ...
        const all = await userCollection.find({}, { projection: { name: 1, userName: 1, _id: 0 } })
        return all.toArray();
    }

    static async getUser(userName: string, proj?: object): Promise<any> {
        const user = await userCollection.findOne({ userName: userName }, { projection: { proj } })
        if (!user) {
            throw new ApiError(`User ${userName} not found`, 404)
        }
        return user
    }
...
async function test() {
    ...
    const kim = await UserFacade.getUser("kim@b.dk")
